...
procedure Menu is
   ...
   procedure HandleInput
     (Pointer          : in String_Access; TextString : in String;
      ...
      CheckKeyPresence : Boolean := False; KeyMustBePresent : Boolean := False)
   is
      -- GET IMPUT FROM USER
         ...
         begin
            ...
            if Input = QuitCharacter then
               ...
               if InputType = INT then
                  ...
                  if TempInt < 0 or
                    (MaxInt /= -1 and (TempInt <= 0 or TempInt > MaxInt))
                  then
                     raise Data_Error;
                  end if;
   ...
   begin
      ...
      begin
         HandleInput
           (Pointer   => NewPersonKey'Access, TextString => "Entrez la clé: ",
            InputType => KEY, CheckKeyPresence => CheckKeyPresence);
         HandleInput
         ...
         HandleInput
           (Pointer    => Birthdate'Access,
            TextString => "Entrez la date d'anniversaire de la personne: ");
         ...
         if Position /= ROOT then
            New_Line;
            Put_Line
              (GetColoredString ("Un nouvel ancêtre a été ajoutée", SUCCESS));
         end if;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            Put_Line
              (GetColoredString
                 ("L'arbre " & To_String (CurrentTree.Name) & " a été crée.",
                  SUCCESS));
         exception
            when OperationAbandonnedException =>
               null;
   ...
   begin
      ...
      begin
         ...
         if UnboundedToInteger (NewVerbosity) /= -1 then
            ...
            Put_Line
              (GetColoredString
                 ("La verbosité a été définie à" & Integer'Image (Verbosity),
                  SUCCESS));
         end if;
      exception
         when OperationAbandonnedException =>
   ...
   procedure TreeMenu is
      ...
      begin
         begin
            ...
            if isEmpty
                (getLeftChild (getNode (Tree, UnboundedToInteger (TargetKey))))
            then
               TargetPosition := LEFT;
            elsif isEmpty
                (getRightChild
      ...
      begin
         begin
            ...
            Put_Line
              (GetColoredString
                 ("L'individu " & To_String (TargetKey) & " a été supprimé.",
                  SUCCESS));
         exception
            when OperationAbandonnedException =>
               New_Line;
         ...
         begin
            ...
            if Ancestors.Is_Empty then
               ...
               Put_Line
                 (GetColoredString
                    ("L'individu " & To_String (TargetKey) & " a " &
                     GetKeysStringFromTreeVector (Ancestors, Verbosity) &
                     " comme ancêtr" &
      ...
      begin
         ...
         if OrphanIndividuals.Is_Empty then
            ...
            Put_Line
              (GetColoredString
                 ("L'individu " &
                  GetKeysStringFromTreeVector (OrphanIndividuals, Verbosity) &
                  " est orphelin.",
                  SUCCESS));
         else
            Put_Line
      ...
      begin
         ...
         if SingleParentIndividuals.Is_Empty then
            Put_Line
              (GetColoredString
                 ("Il n'existe aucun individu avec un seul parent.", WARNING));
         elsif FamilyTree.Length (SingleParentIndividuals) = 1 then
            Put_Line
              (GetColoredString
                 ("L'individu " &
                  GetKeysStringFromTreeVector (SingleParentIndividuals, Verbosity) &
                  " a un seul parent.",
                  SUCCESS));
         else
            Put_Line
      ...
      begin
         ...
         if DualParentIndividuals.Is_Empty then
            Put_Line
              (GetColoredString
                 ("Il n'existe aucun individu avec deux parents.", WARNING));
         elsif FamilyTree.Length (DualParentIndividuals) = 1 then
            Put_Line
              (GetColoredString
                 ("L'individu " &
                  GetKeysStringFromTreeVector (DualParentIndividuals, Verbosity) &
                  " a deux parents.",
                  SUCCESS));
         else
            Put_Line
               ...
               Put_Line
                 (GetColoredString
                    ("L'arbre '" &
                     ...
                     SUCCESS));
               Delete
                 (ExistingTrees, UnboundedToInteger (TreeOperationIndex) - 1);
            end if;
