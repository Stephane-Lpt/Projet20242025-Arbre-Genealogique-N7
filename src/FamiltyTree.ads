...
package FamilyTree is
   ...
   procedure addParent (ABR : in out T_BinaryTree; Id_Node : in Integer; Parent : in T_Person) with
   ...
   function getAncestorsCount (ABR : in T_BinaryTree; Id_Node : in Integer) return Integer;

   -- 4. Obtenir l’ensemble des ancêtres situés à une certaine génération d’un individu donné.
   function getAncestorsByGeneration (ABR : in T_BinaryTree; Id_Node : in Integer; Generation : in Integer) return PersonVector;
   ...
   function getSingleParentIndividuals (ABR : in T_BinaryTree; Id_Node : in Integer) return PersonVector;

   -- 8. Obtenir l’ensemble des individus dont les deux parents sont connus.
   function getDualParentIndividuals (ABR : in T_BinaryTree; Id_Node : in Integer) return PersonVector;

   -- Get node by ID
   function getNode(ABR : in T_BinaryTree; Id_Node : in Integer) return T_Node;
