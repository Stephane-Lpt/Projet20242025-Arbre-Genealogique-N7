...
package body FamilyTree is
   ...
   procedure initChild
     (ABR : out T_FamilyTree; Key : in Integer; Person : in T_Person)
   is
   begin
   ...
   begin
      return
        getSize (Node); -- Utilise la fonction getSize existante du BinaryTree
   ...
   is
      ...
      function Helper
        (ABR : T_FamilyTree; CurrentGen : Integer) return TreeVector.Vector
      is
      ...
      begin
         ...
         declare
            LeftResult  : constant TreeVector.Vector :=
              Helper (getLeftChild (ABR), CurrentGen + 1);
            RightResult : constant TreeVector.Vector :=
              Helper (getRightChild (ABR), CurrentGen + 1);
         begin
   ...
   procedure showFamilyTreeFromId
     (ABR : in T_FamilyTree; Key : in Integer; Verbosity : in Integer := 1)
   is
   begin
   ...
   function getRightChild (ABR : in T_FamilyTree) return T_FamilyTree is
   begin
      return Tree.getRightChild (ABR);
   end getRightChild;

   function getLeftChild (ABR : in T_FamilyTree) return T_FamilyTree is
   begin
      return Tree.getLeftChild (ABR);
   end getLeftChild;

   function getPerson(ABR : in T_FamilyTree) return T_Person is
   begin
      return Tree.getElement(ABR);
   end getPerson;

   function GetKeysStringFromTreeVector
     (Vector : in TreeVector.Vector; Verbosity : in Integer) return String
   is
      KeysString : Unbounded_String := To_Unbounded_String ("");
      Index      : Integer          := 0;
   begin
      if Vector.Is_Empty then
         return To_String (KeysString);
      else
         KeysString := To_Unbounded_String("'");
         for el of Vector loop
            KeysString :=
               KeysString &
               (if Verbosity = 1 then To_Unbounded_String (GetTrimmedInt (getKey (el))) else To_Unbounded_String (GetPersonName(getPerson(el), Verbosity)));
            Index      := Index + 1;
            if Index /= Length (Vector) then
               KeysString := KeysString & To_Unbounded_String (", ");
            end if;
         end loop;
         KeysString := KeysString & To_Unbounded_String("'");
         return To_String (KeysString);
      end if;
   end GetKeysStringFromTreeVector;
